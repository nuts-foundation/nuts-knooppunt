package medicatieoverdracht_test

import rego.v1

import data.medicatieoverdracht
...
test_requester_has_enrolled_patient_valid if {
	medicatieoverdracht.requester_has_enrolled_patient with input as base_input
}

test_requester_has_enrolled_patient_bsn_mismatch if {
	not medicatieoverdracht.requester_has_enrolled_patient with input as object.union(base_input, {
...
test_allow_medication_request_search if {
	medicatieoverdracht.allow with input as base_input
}

test_deny_without_capability_checked if {
	not medicatieoverdracht.allow with input as object.union(base_input, {
		"action": {"fhir_rest": {"capability_checked": false}},
	})
}

test_deny_without_mitz_consent if {
	not medicatieoverdracht.allow with input as object.union(base_input, {"context": {"mitz_consent": false}})
}
...
test_allow_with_patient_bsn_identifier if {
	medicatieoverdracht.allow with input as object.union(base_input, {"context": {"patient_id": ""}})
}

test_deny_wrong_resource_type if {
	not medicatieoverdracht.allow with input as object.union(base_input, {"resource": {"type": "Patient"}})
}
