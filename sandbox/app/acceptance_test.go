...
func acceptanceServer(t *testing.T) (harness.Details, *httptest.Server, *http.Client, Config) {
	t.Helper()
	h := harness.Start(t)
	...
	t.Cleanup(srv.Close)
	return h, srv, signInViaDezi(t, srv), cfg
}
...
func TestAcceptance_ShareRegistersBothAndConfirms(t *testing.T) {
	h, srv, client, _ := acceptanceServer(t)
	...
	for _, list := range plataanLists {
		...
		require.Equal(t, coding.BSNTransportTokenNamingSystem, *list.Subject.Identifier.System,
			"the NVI must identify the patient by its own identifier system, not by BSN")
		require.NotEqual(t, anna.BSN, *list.Subject.Identifier.Value,
			"the NVI must hold a pseudonym, never the BSN itself")
	}
...
func TestAcceptance_ASecondSessionCannotOpenALockedPatient(t *testing.T) {
	_, srv, client, _ := acceptanceServer(t)
	anna, ok := pool.PatientByKey("anna")
	require.True(t, ok)
