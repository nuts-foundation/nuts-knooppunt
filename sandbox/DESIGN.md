...
| --- | --- | --- | --- |
| **GF Sandbox** | Shell (path chooser, demo controls, reset) whose /demo path renders the consumer EMR experience: **Plataan EHR**, the hospital system of Ziekenhuis De Plataan. Node backend proxies to the Knooppunt and emits step events | Shell: quiet frame (6.1). EMR: warm editorial (6.2) | own port, e.g. :8091 (the current demo-ehr maps host :8091 to container :3000 and is to be split; host :3000 itself is taken by the mock VC issuer) |
| **ZorgDossier** | The elderly care institution's own record system: resident record, data entry, NVI registration on save. Reuses demo-ehr's backend/proxy machinery, restyled and reduced | Own brand, deliberately different (6.3) | own port, e.g. :3001 |
...
1. **Login (GF Authenticatie): the specialist logs into the Plataan EHR via a mock Dezi login.** From here the EMR top bar permanently shows the Dezi session: practitioner name, role, UZI number, and organization context ("Logged in to: De Plataan Hospital").
