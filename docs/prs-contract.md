...
| **Service `main`, v0.0.34** | Same repository, `main` at `db1160b` (2026-09-08); latest tag `v0.0.34` (commit `ea91e01`, 2026-09-04). | Not deployed to acceptance. Breaks the current client in two places (organization identifier, scope), which arrived in different tags; see the checklist. |
...
element encoding, and scalar multiplication with the little-endian scalar
encoding, which is everything the blind, the evaluation and the de-blinding
consist of.
...
zBytes, _ := g.NewElement().Mul(p, k).MarshalBinary()
```
...
#include <sodium.h>
#include "oprf.h"
uint8_t p[crypto_core_ristretto255_BYTES], z[crypto_core_ristretto255_BYTES];
...
| `test/prsmock` (`TestService_Eval`, `TestService_Transport`, `Test_pythonURLSafeB64Decode`) | That the mock behaves as its package comment says: the v0.0.18 checks it models, in their order, with their status codes and bodies; the TLS, client-certificate and bearer-token requirements; and its model of Python's base64 decoder, against values from CPython. |
