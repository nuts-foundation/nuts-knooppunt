...
| Dual-assertion token request (`assertion` + `client_assertion`) | 10.9.6 | Out of scope                          | Out of scope — `client_assertion` accepted but ignored | —                                                                                  |
| `PatientEnrollmentCredential` validation                      | 10.6.5   | Supported, not validated — TBD        | Supported, not validated — TBD                         | —                                                                                  |
| `HealthcareOrganizationCredential` validation                 | 10.6.3   | Supported, not validated — TBD        | Supported, not validated — TBD                         | [nuts-node#4079](https://github.com/nuts-foundation/nuts-node/issues/4079)         |
| `HealthCareProfessionalDelegationCredential` validation       | 10.6.4   | Supported, not validated — TBD        | Supported, not validated — TBD                         | —                                                                                  |
