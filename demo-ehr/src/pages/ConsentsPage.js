...
function ConsentsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if ((form.provisionActorsOrgRefs || []).length === 0) {
  ...
  return (
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        {loading ? (
          <div className="loading-container"><div className="spinner"/><p>Loading consents...</p></div>
        ) : error ? (
          ...
          <div className="patients-table-container" style={{ marginTop:'20px' }}>
            <table className="patients-table">
              ...
              <tbody>
                {consents.map(c => (
                  <tr key={c.id} style={{ cursor:'pointer' }} onClick={() => openEdit(c)} title="Edit consent">
                    <td>{c.status}</td>
                    <td>{c.patient?.reference || '-'}</td>
                    <td>{(c.provision?.actor||[]).map(a=>a.reference?.reference).join(', ') || '-'}</td>
                    <td>{c.provision?.type || '-'}</td>
          ...
          <div className="modal-overlay">
            <div className="modal" style={{ maxWidth:'640px' }}>
              ...
              <form onSubmit={handleSubmit} className="new-patient-form">
                ...
                <div className="form-row">
                  <label>Patient</label>
                  <select value={form.patientReference} onChange={e=>setForm(f=>({ ...f, patientReference:e.target.value }))} required>
                    <option value="">-- select patient --</option>
                    {patients.map(p => (
                      <option key={p.id} value={`Patient/${p.id}`}>{patientApi.formatName(p)} ({p.id})</option>
                    ))}
                  </select>
                </div>
                <div className="form-row">
                  <label>Provision Actor Orgs</label>
                  <div className="checkbox-grid" style={{ display:'grid', gap:'6px', gridTemplateColumns:'repeat(auto-fill, minmax(220px,1fr))' }}>
                    {organizations.map(o => {
                      const ref = `Organization/${o.id}`;
                      return (
                        <label key={o.id} style={{ fontWeight:'normal' }}>
                          <input type="checkbox" checked={form.provisionActorsOrgRefs.includes(ref)} onChange={()=>toggleArrayValue('provisionActorsOrgRefs', ref)} /> {o.name || ref}
                        </label>
                      );
                    })}
                  </div>
                ...
                <div className="form-row">
                  ...
                  <select value={form.provisionType} onChange={e=>setForm(f=>({ ...f, provisionType:e.target.value }))}>
                ...
                <div className="form-row">
                  ...
                  <input type="text" value={form.categoryCodes.map(c=>c.code).join(',')} onChange={e=>{
                ...
                <div className="form-actions" style={{ justifyContent:'space-between' }}>
                  <div style={{ display:'flex', gap:'8px' }}>
                    <button type="button" className="button button-secondary" onClick={closeModal} disabled={saving || deleting}>Cancel</button>
                    <button type="submit" className="button" disabled={saving || deleting}>{saving ? 'Saving...' : (editingConsent ? 'Save Changes' : 'Create')}</button>
                  </div>
                  {editingConsent && (
                    <button type="button" className="button button-danger" onClick={handleDelete} disabled={saving || deleting}>{deleting ? 'Deleting...' : '🗑 Delete'}</button>
                  )}
                </div>
              </form>
            </div>
