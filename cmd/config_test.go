...
func TestLoadConfig_Default(t *testing.T) {
	...
	assert.Equal(t, "", config.Nuts.ConfigFile)
...
nuts:
  enabled: false
  configfile: "test-nuts.yaml"
`

	configFile := filepath.Join(configDir, "knooppunt.yaml")
	err = os.WriteFile(configFile, []byte(yamlContent), 0644)
	require.NoError(t, err)

	// Change to temp directory so config/knooppunt.yaml is found
	...
	assert.Equal(t, "test-nuts.yaml", config.Nuts.ConfigFile)
...
func TestLoadConfig_FromEnvironmentVariables(t *testing.T) {
	...
	t.Setenv("KNPT_NUTS_CONFIGFILE", "env-nuts.yaml")
	...
	assert.False(t, config.Nuts.Enabled)
	assert.Equal(t, "env-nuts.yaml", config.Nuts.ConfigFile)
...
`
	...
	assert.Equal(t, "yaml-nuts.yaml", config.Nuts.ConfigFile) // from YAML (no env override)
